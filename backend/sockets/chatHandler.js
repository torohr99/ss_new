  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_game', async (data, callback) => {
      ...
      try {
        ...
        if (gameSummary && gameSummary.header && gameSummary.header.competitions) {
          const comp = gameSummary.header.competitions[0];
          const gameEspnIds = comp.competitors.map(c => String(c.team.id));
          ...
          messages = messages.map(msg => {
            ...
            const matchedTeam = uTeams.find(ut => gameEspnIds.includes(String(ut.team.espn_id)));
            return {
              ...msg,
              userTeamBadge: matchedTeam ? { abbreviation: matchedTeam.team.name, color: matchedTeam.team.primaryColor || '#888' } : { abbreviation: 'Neutral', color: '#666' }
            };
          });
        }
    ...
    socket.on('send_message', async (data, callback) => {
      ...
      try {
        ...
        if (socket.gameContext && socket.gameContext.competitors) {
          ...
          const gameEspnIds = socket.gameContext.competitors.map(c => String(c.team.id));
          const matchedTeam = userTeams.find(ut => gameEspnIds.includes(String(ut.team.espn_id)));
          if (matchedTeam) {
            teamId = matchedTeam.team.id;
            userTeamBadge = { abbreviation: matchedTeam.team.name, color: matchedTeam.team.primaryColor || '#888' };
          } else {
            userTeamBadge = { abbreviation: 'Neutral', color: '#666' };
          }
        }
