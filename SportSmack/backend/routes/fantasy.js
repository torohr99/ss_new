  ...
  try {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      if (!bot) {
        ...
        const botHash = await bcrypt.hash('bot_password_123', 10);
    ...
    // Set Draft Order by shuffling then updating each team's draftOrder
    ...
    res.status(500).json({ error: 'Failed to create test league', detail: err.message });
  }
});
