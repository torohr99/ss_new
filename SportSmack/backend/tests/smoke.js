...
async function main() {
  ...
  await runTest(
    ...
    async () => {
      ...
      if (result.body.database !== 'OK') {
        throw new Error(`Expected database OK, received ${result.body.database}`);
      }
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      if (result.body.redis !== 'OK') {
        throw new Error(`Expected Redis OK, received ${result.body.redis}`);
      }
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      if (!result.body.instance) {
        throw new Error('Health endpoint did not report an instance.');
      }
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      if (result.headers.get('x-frame-options') !== 'DENY') {
        throw new Error('X-Frame-Options header is missing or incorrect.');
      }
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      if (result.status !== 401) {
        throw new Error(`Expected 401, received ${result.status}`);
      }
    }
  );
  ...
  await runTest(
    ...
    async () => {
      const result = await request('/api/this-endpoint-does-not-exist');
  ...
  await runTest(
    ...
    async () => {
      const result = await request('/api/posts', {
        ...
        body: JSON.stringify({
          content: 'Automated security test'
        })
      });
  ...
  await runTest(
    ...
    async () => {
      const result = await request('/api/moderation/blocks');
  ...
  await runTest(
    ...
    async () => {
      const result = await request('/api/admin/moderation/reports');
  ...
  await runTest(
    ...
    async () => {
      const result = await request('/api/posts', {
        ...
        body: JSON.stringify({
          content: 'CSRF security test'
        })
      });

      if (result.status !== 403) {
        throw new Error(`Expected 403, received ${result.status}`);
      }
    }
  );
    ...
    await runTest(
      ...
      async () => {
        const result = await request('/api/posts', {
          ...
          body: JSON.stringify({
            content: 'Automated CSRF origin test'
          })
        });
